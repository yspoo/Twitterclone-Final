...
# grab the username from the URL as :id
    if User.find_by_username(params[:id]) # this is to find a User by his/her username(which is unique because of our database validations) using User ID, which is unique by default(database behaviour).
      @username = params[:id] # this is to change the User ID(which is a number) into a instance variable @username.
